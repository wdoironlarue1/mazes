...
class kruskal extends Algorithm {
    ...
    execute = async () => {
        ...
        while (this.edges.length !== 0) {
            ...
            if(edge.direction === EDGE_ACROSS && (!this.cells[edge.y * this.width + edge.x].set.includes((edge.y + 1) * this.width + edge.x))) {
                    ...
                    this.breakWall({x: edge.x, y: edge.y}, {x: edge.x, y: edge.y + 1});
                    if(!this.finish) {
            ...
            if (edge.direction === EDGE_VERTICAL && (!this.cells[edge.y * this.width + edge.x].set.includes(edge.y * this.width + edge.x + 1))) {
                    ...
                    this.breakWall({x: edge.x, y: edge.y}, {x: edge.x + 1, y: edge.y});
                    if(!this.finish) {
                        await new Promise(r => setTimeout(r, this.runSpeed));
                    }
                    if (this.stop){
    ...
    step = () => {
        ...
        for(let i = 0; i < this.edges.length; i++) {
            ...
            if(
                (edge.direction === EDGE_ACROSS &&
                    (!this.cells[edge.y * this.width + edge.x].set.includes((edge.y + 1) * this.width + edge.x)))
                    || (edge.direction === EDGE_VERTICAL && 
                    (!this.cells[edge.y * this.width + edge.x].set.includes(edge.y * this.width + edge.x + 1)))
                ) {
